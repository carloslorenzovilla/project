...
class User(db.Model, UserMixin):
    ...
    transactions = db.relationship('Log', backref='user', lazy='dynamic')
    recommendations = db.relationship(
        'Rec', backref='user', lazy='dynamic')
...
class Log(db.Model):
    ...
    def __repr__(self):
        return f"{self.date.strftime('%m-%d-%Y')}: {Item.query.filter_by(id=self.item_id).first()}"
...
class Rec(db.Model):
    ...
    def __repr__(self):
        return f"{self.date.strftime('%Y-%m-%d')}: {Item.query.filter_by(id=self.item_id).first()}"
